...
impl<'a> RunawayVector<'a> {
    ...
    pub fn select0(&self, mut rank: usize) -> Option<usize> {
        ...
        let mut u8_pos = 0;
        while rank > 8 && u8_pos < 64 {
            let ones = self.bit_vec
                [bit_search_start + 8 * u8_pos..bit_search_start + 8 * (u8_pos + 1)]
                .count_zeros();
            rank -= ones;
            u8_pos += 1;
        }
        ...
        let mut bit = 0;
        for i in bit_search_start + 8 * u8_pos..bit_search_end {
            if !self.bit_vec[i] {
                rank -= 1;
                if rank == 0 {
                    bit = i;
    ...
    pub fn select1(&self, mut rank: usize) -> Option<usize> {
        ...
        // Fast 8 bit search, should count ones with popcount
        let mut u8_pos = 0;
        while rank > 8 && u8_pos < 64 {
            let ones = self.bit_vec
                [bit_search_start + 8 * u8_pos..bit_search_start + 8 * (u8_pos + 1)]
        ...
        // Slow search inside an 8 bit block
        let mut bit = 0;
        for i in bit_search_start + 8 * u8_pos..bit_search_end {
            if self.bit_vec[i] {
                rank -= 1;
