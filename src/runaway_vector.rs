...
pub struct RunawayVector<'a> {
    ...
    one_l12_indices: Vec<InterleavedIndex>,
    one_l0_indices: Vec<u64>,
    zero_l12_indices: Vec<InterleavedIndex>,
    zero_l0_indices: Vec<u64>,
}
...
impl<'a> RunawayVector<'a> {
    pub fn new(bit_vec: &'a BitVec<u64, Lsb0>) -> Self {
        ...
        let mut one_l0_indices: Vec<u64> = Vec::with_capacity(l0_capacity);
        let mut zero_l0_indices: Vec<u64> = Vec::with_capacity(l0_capacity);

        let mut one_l12_indices: Vec<InterleavedIndex> = Vec::with_capacity(l12_capacity);
        let mut zero_l12_indices: Vec<InterleavedIndex> = Vec::with_capacity(l12_capacity);

        let mut one_l0: u64 = 0;
        let mut zero_l0: u64 = 0;
        let mut one_l1: u32 = 0;
        let mut zero_l1: u32 = 0;
        let mut one_l2s: [u16; 4] = [0; 4];
        let mut zero_l2s: [u16; 4] = [0; 4];
        let mut l2_len = 0;
        for (i, chunk) in bit_vec.chunks(L2_BIT_SIZE).enumerate() {
            let l2_pos = i % 4;
            let one_l2 = chunk.count_ones() as u16;
            let zero_l2 = L2_BIT_SIZE as u16 - one_l2;
            one_l2s[l2_pos] = one_l2;
            zero_l2s[l2_pos] = zero_l2;
            l2_len += 1;
            if i % (L1_BIT_SIZE / L2_BIT_SIZE) == 3 {
                one_l12_indices.push(InterleavedIndex::new(one_l1, &one_l2s[0..l2_len - 1]));
                zero_l12_indices.push(InterleavedIndex::new(zero_l1, &zero_l2s[0..l2_len - 1]));
                one_l1 = one_l1 + one_l2s[0] as u32 + one_l2s[1] as u32 + one_l2s[2] as u32 + one_l2s[3] as u32;
                zero_l1 = zero_l1 + zero_l2s[0] as u32 + zero_l2s[1] as u32 + zero_l2s[2] as u32 + zero_l2s[3] as u32;
                one_l2s = [0; 4];
                zero_l2s = [0; 4];
                l2_len = 0;
            }
            if i % (L0_BIT_SIZE / L2_BIT_SIZE) == (L0_BIT_SIZE / L2_BIT_SIZE) - 1 {
                one_l0_indices.push(one_l0);
                zero_l0_indices.push(zero_l0);
                one_l0 += one_l1 as u64;
                zero_l0 += zero_l1 as u64;
                one_l1 = 0;
                zero_l1 = 0;
            }
        }
        if bit_vec.len() % L1_BIT_SIZE != 0 {
            one_l12_indices.push(InterleavedIndex::new(one_l1, &one_l2s[0..l2_len]));
            zero_l12_indices.push(InterleavedIndex::new(zero_l1, &zero_l2s[0..l2_len]));
        }
    ...
    pub fn space_usage(&self) -> usize {
        (self.one_l12_indices.len() * 64 + self.one_l0_indices.len() * 64) * 2 + mem::size_of::<Self>()
    }
...
impl<'a> Selectable for RunawayVector<'a> {
    fn select0(&self, nth: usize) -> Option<usize> {
        ...
        let mut rank = nth;
        let mut l0_index = 0;
        while l0_index < self.zero_l0_indices.len() {
            if self.zero_l0_indices[l0_index] as usize >= nth {
                l0_index -= 1;
                break;
            } else if l0_index + 1 == self.zero_l0_indices.len() {
                break;
            }
            l0_index += 1;
        }
        rank -= self.zero_l0_indices[l0_index] as usize;

        let first_l1 = l0_index * L0_BIT_SIZE / L1_BIT_SIZE;
        let last_l1 = if (l0_index + 1) * (L0_BIT_SIZE / L1_BIT_SIZE) > self.zero_l12_indices.len() - 1 {
            self.zero_l12_indices.len() - 1
        } else {
        ...
        while l <= r {
            let m = (l + r) / 2;
            if (self.zero_l12_indices[m].l1() as usize) < rank {
                l = m + 1;
            } else if (self.zero_l12_indices[m].l1() as usize) >= rank {
                r = m - 1;
            } else {
                break;
            }
        ...
        let l1_index = r;
        let l12_index = &self.zero_l12_indices[l1_index];
    ...
    fn select1(&self, nth: usize) -> Option<usize> {
        ...
        let mut rank = nth;
        let mut l0_index = 0;
        while l0_index < self.one_l0_indices.len() {
            if self.one_l0_indices[l0_index] as usize >= nth {
                l0_index -= 1;
                break;
            } else if l0_index + 1 == self.one_l0_indices.len() {
                break;
            }
            l0_index += 1;
        }
        rank -= self.one_l0_indices[l0_index] as usize;

        let first_l1 = l0_index * L0_BIT_SIZE / L1_BIT_SIZE;
        let last_l1 = if (l0_index + 1) * (L0_BIT_SIZE / L1_BIT_SIZE) > self.one_l12_indices.len() - 1 {
            self.one_l12_indices.len() - 1
        } else {
            (l0_index + 1) * (L0_BIT_SIZE / L1_BIT_SIZE)
        };
        ...
        while l <= r {
            let m = (l + r) / 2;
            if (self.one_l12_indices[m].l1() as usize) < rank {
                l = m + 1;
            } else if (self.one_l12_indices[m].l1() as usize) >= rank {
                r = m - 1;
            } else {
                break;
            }
        ...
        let l1_index = r;
        let l12_index = &self.one_l12_indices[l1_index];
        rank -= l12_index.l1() as usize;
        let mut l2_index = 0;
        for i in 0..l12_index.len() {
            if rank > l12_index.index(i) as usize {
...
impl<'a> Rankable for RunawayVector<'a> {
    fn rank1(&self, idx: usize) -> usize {
        ...
        let l0: usize = self.one_l0_indices[l0_pos] as usize;
        let l12 = &self.one_l12_indices[l1_pos];
...
mod tests {
    ...
    fn small_vector() {
        ...
        assert_eq!(runaway.one_l0_indices.len(), 1);
        assert_eq!(runaway.one_l0_indices[0], 0);
        assert_eq!(runaway.one_l12_indices.len(), 1);
        assert_eq!(runaway.one_l12_indices[0].l1(), 0);
        assert_eq!(runaway.one_l12_indices[0].len(), 3);
        assert_eq!(runaway.one_l12_indices[0].index(0), 512);
        assert_eq!(runaway.one_l12_indices[0].index(1), 512);
        assert_eq!(runaway.one_l12_indices[0].index(2), 256);
    }
    ...
    fn full_vector() {
        ...
        let runaway = RunawayVector::new(&bv);
        assert_eq!(runaway.one_l0_indices.len(), 2);
        assert_eq!(runaway.one_l0_indices[0], 0);
        assert_eq!(runaway.one_l0_indices[1], u32::MAX as u64);
    }
    ...
    fn select1_l0() {
        ...
        let runaway = RunawayVector::new(&bv);
        assert_eq!(runaway.one_l0_indices[0], 0);
        assert_eq!(runaway.one_l0_indices[1], 1);
        assert_eq!(runaway.one_l0_indices[2], 2);
    ...
    fn select1_l1() {
        ...
        let runaway = RunawayVector::new(&bv);
        assert_eq!(runaway.one_l0_indices[0], 0);
        assert_eq!(runaway.one_l12_indices.len(), 4);
        assert_eq!(runaway.select1(1), Some(0));
